...
const IncomeYearAnalytic = ({title, deposits=[], expenses=[], year = 2019}) => {
    ...
    useEffect(()=>{
        if(deposits.length !== 0 || expenses.length !== 0) {
            let dates = valueToDate(value)
            let sortedDeposits = analyticService.sortByMonths(deposits, dates.startDate, dates.endDate)
            let sortedExpenses = analyticService.sortByMonths(expenses, dates.startDate, dates.endDate)
            setGraphData({
                depositData: analyticService.toLineGraphData(sortedDeposits)[1],
                expenseData: analyticService.toLineGraphData(sortedExpenses)[1],
    ...
    const handleChange = (event, newValue) => {
        if (!equals(value, newValue)) {
            let dates = valueToDate(newValue)
            let sortedDeposits = analyticService.sortByMonths(deposits, dates.startDate, dates.endDate)
            let sortedExpenses = analyticService.sortByMonths(expenses, dates.startDate, dates.endDate)
            setGraphData({
                depositData: analyticService.toLineGraphData(sortedDeposits)[1],
    ...
    return (
        ...
        ?<div className={classes.container}>
            ...
            <LineGraph
                classes={classes}
                depositData={graphData.depositData}
                expenseData={graphData.expenseData}
...
export default React.memo(IncomeYearAnalytic)